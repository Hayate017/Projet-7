<template>
    <div class="userCard">
        ...
        <div class="userCard__role__bloc userCard__bloc">
            <button class="userCard__change__role__btn" v-if="loggedUserRole === 'admin' && this.role != 'admin'"
                v-on:click="changeRole">{{ role }}</button>
        ...
        <div class="userCard__delete__bloc userCard__bloc"
            v-if="loggedUserRole === 'admin' && loggedUserId !== this.id">
            <i v-on:click="deleteProfile" v-on:keydown.enter="deleteProfile" class="fa-solid fa-trash userCard__icon"
                role="button" tabindex="0"></i>
        </div>

        <userDeletionModale @deleteUser="updateUsersList" v-bind:id="id" v-bind:showDeletionModale="showDeletionModale"
