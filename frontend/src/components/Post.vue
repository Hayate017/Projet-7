<template>
    <div class="post">
        <div class="post__management"
            v-if="user_Id === loggedUserId || loggedUserRole === 'admin' || loggedUserRole === 'modérateur'">
            <div v-on:click="showPostManagment = !showPostManagment"
                v-on:keydown.enter="showPostManagment = !showPostManagment" class="post__management__btn" role="button"
                tabindex="0">
                <i class="fa-solid fa-ellipsis"></i>
            </div>
            <div v-if="showPostManagment" class="post__management__container">
                <div v-on:click="showEditModale = !showEditModale" v-on:keydown.enter="showEditModale = !showEditModale"
                    v-if="user_Id === loggedUserId" class="post__btn__bloc" role="button" tabindex="0">
                    <i class="fa-solid fa-pen-to-square"></i>
                </div>
                <div v-on:click="toPostDeletion" v-on:keydown.enter="toPostDeletion" class="post__btn__bloc"
                    role="button" tabindex="0">
                    <i class="fa-solid fa-trash"></i>
                </div>
            </div>
        </div>
        <p v-if="user_Id != loggedUserId" class="post__username__info">Publié par <span class="post__username">{{
                username
        }}</span></p>
        ...
        <div class="post__btn__bloc__container">
            <div class="post__btn__bloc">
                <i v-bind:class="{ orange: postLiked }" v-on:click="likePost" v-on:keydown.enter="likePost"
                    class="fa-solid fa-thumbs-up" role="button" tabindex="0"></i><span class="count">{{ likesCount +
                            likes
                    }}</span>
            </div>
            <div class="post__btn__bloc">
                <i v-on:click="showCommentModale = !showCommentModale"
                    v-on:keydown.enter="showCommentModale = !showCommentModale" class="fa-solid fa-comment"
                    role="button" tabindex="0"></i><span class="count">{{ commentsCount }}</span>
            </div>
        </div>

        <editPostForm @closeEditModale="closeEditModale" @updatePostList="updatePostList"
            v-bind:showEditModale="showEditModale" v-bind:id="id">
        </editPostForm>

        <postDeletionModale @updatePostList="updatePostList" v-bind:showDeletionModale="showDeletionModale"
            v-bind:token="token" v-bind:id="id" v-on:closeModale="closeModale">
        </postDeletionModale>

        <commentModale @commentsCount="getCommentsCount" v-bind:showCommentModale="showCommentModale" v-bind:id="id"
